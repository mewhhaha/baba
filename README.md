...
non-empty editor query fragments, while `--target all` emits both backends. The
...
`grammar.js`, or `--target all` to emit them with both parser backends.
...
- `--target wasm|tree-sitter|all` selects generated backends.
...
regenerate plans produced by earlier Baba versions. See
[Migrating to Baba 5](docs/migrating-to-5.md) for the complete upgrade steps.
