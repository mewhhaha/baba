...
lexer-scan/parser-trace and parser expected-range conformance. The Stage-0
compiler lowers validated runtime-language programs to one resolved
control-flow/value IR before the TypeScript and Wasm backends emit target
artifacts; target-specific source/byte emission is still separate.
