...
- Moved public rule-node child-list classification onto runtime-language
  `parserRuleNodeChildListStatus`, so the shared rule-node materializer no
  longer directly decides that a zero child count means an empty child array.
...
- Moved generated canonical token identity matching onto runtime-language
  `parserTokenStreamTokenMatchStatus`, so `parseTokens()` no longer directly
  compares public token type/kind/literal/span fields in TypeScript when
  checking supplied tokens against whole-source lexer output.
- Moved generated canonical token stream advancement onto runtime-language
  `parserTokenStreamCanonicalMatchStatus`, so `parseTokens()` no longer directly
  decides in TypeScript whether a canonical token matches the supplied token,
  can be skipped as omitted trivia, or should fail matching.
- Moved generated external token-stream final EOF/trailing-gap classification
  onto runtime-language `parserTokenStreamFinalStatus`, so `parseTokens()` no
  longer directly decides in TypeScript whether EOF is the last supplied token
  or a no-EOF stream leaves unvalidated source text.
- Moved generated external token-stream public literal/named/error shape
  classification onto runtime-language `parserTokenStreamPublicTokenStatus`, so
  `parseTokens()` no longer directly decides in TypeScript whether literal
  channel/text, named channel, error channel, or unknown public token tags are
  valid.
...
- Moved generated parser replay RHS stack-slice start arithmetic onto
  runtime-language `parserReplayRhsStart`, so generated TypeScript no longer
  directly special-cases empty reductions or subtracts RHS length from the host
  value stack before calling `splice`.
- Moved generated parser replay sentinel stack-depth arithmetic onto
