...
Deno.test("grammar v2 incremental parser matches full CST after edit", () => {
  ...
  const full = buildGrammarV2TokenCst(schema, lexer, next.source);
  ...
  assertEquals(debugGrammarV2Cst(next.tree), debugGrammarV2Cst(full.root));
});

Deno.test("grammar v2 incremental parser refreshes diagnostics after error edit", () => {
...
Deno.test("grammar v2 incremental parser cancellation returns old state safely", () => {
  const { parser } = parserFixture();
  ...
  assertEquals(debugGrammarV2Cst(next.tree), debugGrammarV2Cst(initial.tree));
});
