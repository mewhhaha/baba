...
Deno.test("grammar AST materializer builds clean nodes from annotations", () => {
  const analyzed = astGrammar();
  const lexer = buildGrammarLexerPlan(analyzed);
  const result = materializeGrammarAst(
    analyzed,
    lexer,
    "let x = 1; let y = x;",
  );
...
Deno.test("grammar AST materializer returns invalid placeholder for broken input", () => {
  const analyzed = astGrammar();
  const schema = buildGrammarAstSchema(analyzed);
  assertEquals(schema.diagnostics.length, 0);
  const lexer = buildGrammarLexerPlan(analyzed);
  const result = materializeGrammarAst(analyzed, lexer, "let x = ;");
...
function astGrammar(): AnalyzedGrammar {
...
function assertAstNode(
  value: GrammarAstValue,
): asserts value is GrammarAstNode {
...
function publicAst(value: GrammarAstValue): unknown {
  if (value === undefined) {
    return undefined;
  }
  if (isAstArray(value)) {
...
function isAstArray(
  value: GrammarAstValue,
): value is readonly GrammarAstValue[] {
