...
import type { Diagnostic, GrammarDocument } from "./helpers.ts";

Deno.test("grammar parser accepts the task syntax example", async () => {
  const source = await Deno.readTextFile(
    "fixtures/grammar/valid/core.grammar",
  );
...
Deno.test("grammar parser accepts token priority declarations", () => {
  ...
  assertEquals(result.diagnostics.length, 0);
  assert(result.grammar);
  const token = result.grammar.declarations[0];
  assertEquals(token.kind, "token");
  if (token.kind === "token") {
    assertEquals(token.priority, 10);
  }
});

Deno.test("grammar parser covers imports and separated-list expressions", async () => {
  const source = await Deno.readTextFile(
    "fixtures/grammar/valid/lists.grammar",
  );
  const result = parseGrammar(source);
...
Deno.test("grammar parser covers modes, transitions, layout, modules, and extensions", async () => {
  const source = await Deno.readTextFile(
...
Deno.test("grammar parser reports multiple syntax diagnostics with spans", async () => {
  const source = await Deno.readTextFile(
    "fixtures/grammar/invalid/broken.grammar",
  );
  const result = parseGrammar(source);
  ...
  for (const diagnostic of result.diagnostics) {
    assertEquals(diagnostic.code, "GRAMMAR_PARSE_ERROR");
...
Deno.test("grammar fixture snapshots stay stable without source spans", async () => {
  const validFixtures = [
    "fixtures/grammar/valid/core.grammar",
    "fixtures/grammar/valid/lists.grammar",
    "fixtures/grammar/valid/modes-layout-modules.grammar",
  ...
  for (const path of validFixtures) {
    const result = parseGrammar(await Deno.readTextFile(path));
    assertEquals(result.diagnostics.length, 0);
    assert(result.grammar);
    validSnapshots[path] = normalizeGrammar(result.grammar);
  }
  assertEquals(
  ...
  const invalid = parseGrammar(
    await Deno.readTextFile("fixtures/grammar/invalid/broken.grammar"),
  );
  assertEquals(
...
function normalizeGrammar(grammar: GrammarDocument): unknown {
