...
Deno.test("grammar v2 modules compose exported extension alternatives deterministically", () => {
  const core = grammar(`
    grammar Core
  ...
  const composed = composeGrammarV2Modules([core, tests]);
  ...
  const plan = buildGrammarV2ParserCorePlan(composed.analyzed);
  ...
  assertEquals(validateGrammarV2Parse(plan, "let a;").ok, true);
  assertEquals(validateGrammarV2Parse(plan, "test b;").ok, true);
});

Deno.test("grammar v2 modules reject extension points that are not exported", () => {
  const core = grammar(`
    grammar Core
  ...
  const composed = composeGrammarV2Modules([core, effects]);
...
Deno.test("grammar v2 modules report import cycles", () => {
  const left = grammar(`
    grammar Left
    import Right ;
    export item
  ...
  const composed = composeGrammarV2Modules([left, right]);
