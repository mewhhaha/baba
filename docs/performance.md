...
literal overlap checks, combined lexer DFA construction, BNF lowering, canonical
LR table construction, contextual token analyses, portable plan encoding and
hashing, Wasm planning, and bundle emission. The planning observer is supplied
only by the benchmark, so ordinary compilation does not read the clock or
allocate timing records.
...
The runtime benchmark reports generated bytes, plan statistics, import time,
Wasm module compile time, parser creation, compiler and bundle-write time, cold
calls, alternating hot-sample distributions, raw Wasm memory pages, and plan
sizes for the runtime fixtures. Each fixture is compiled once; the same bundle
supplies artifact inspection, disk output, module import, and runtime
measurements.
