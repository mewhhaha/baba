...
expected-terminal display ranges for diagnostics, and `parserExpectedHasEof`
flags choose trailing-input diagnostic codes without scanning display strings.
...
- Generated parser modules export numeric parser diagnostic code constants
  ...
  for the parser state that produced the expected-terminal set.
