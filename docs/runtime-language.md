...
and generated Wasm JavaScript adapters. Public lex result objects are allocated
...
`parserFieldFinalStatus` helpers. Replay reduction Array-field capture assembly
also uses `parserFieldArrayValueStatus` before appending to the runtime vector.
Replay reduction validity, sentinel stack-depth calculation, and RHS stack
slicing use `parserReplayReductionStatus`, `parserReplayStackDepth`, and
`parserReplayRhsStart`; span/token-range merge arithmetic uses
