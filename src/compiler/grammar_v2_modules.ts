...
import { analyzeGrammarV2 } from "./grammar_v2_analysis.ts";

export interface GrammarV2ModuleCompositionResult {
  readonly document: GrammarV2Document;
  readonly analyzed: AnalyzedGrammarV2;
  readonly diagnostics: readonly GrammarV2ModuleDiagnostic[];
  readonly moduleOrder: readonly string[];
}

export interface GrammarV2ModuleDiagnostic extends Diagnostic {
...
export function composeGrammarV2Modules(
  documents: readonly GrammarV2Document[],
): GrammarV2ModuleCompositionResult {
  const diagnostics: GrammarV2ModuleDiagnostic[] = [];
  if (documents.length === 0) {
    const emptySpan = { start: 0, end: 0, line: 1, column: 1 };
    const document = { declarations: [], span: emptySpan };
    const analyzed = analyzeGrammarV2(document);
  ...
  const composedDeclarations: GrammarV2Declaration[] = [];
  const composedRules = new Map<string, GrammarV2Rule>();
  ...
  const document: GrammarV2Document = {
...
function moduleEntry(document: GrammarV2Document): ModuleEntry {
  let name = "anonymous";
...
function topologicalModules(
  ...
  diagnostics: GrammarV2ModuleDiagnostic[],
...
function appendAlternative(
  existing: GrammarV2Expression,
  extension: GrammarV2Expression,
): GrammarV2Expression {
  const options: GrammarV2Expression[] = [];
