...
interface AnalyzeGrammarV2Options {
  readonly rootRule?: string;
}

interface AnalyzerContext {
  readonly diagnostics: Diagnostic[];
  readonly modes: AnalyzedGrammarV2Mode[];
  readonly tokens: AnalyzedGrammarV2Token[];
  readonly rules: AnalyzedGrammarV2Rule[];
  readonly literals: AnalyzedGrammarV2Literal[];
  readonly expressionIslands: AnalyzedGrammarV2ExpressionIsland[];
  readonly constructors: AnalyzedGrammarV2Constructor[];
  readonly fields: AnalyzedGrammarV2Field[];
  readonly modulesByName: Map<string, SourceSpan>;
  readonly exportsByName: Map<string, SourceSpan>;
  readonly tokensByName: Map<string, GrammarV2TokenId>;
  readonly modesByName: Map<string, GrammarV2ModeId>;
  readonly rulesByName: Map<string, GrammarV2RuleId>;
  readonly syncByRuleName: Map<string, GrammarV2Expression>;
  readonly literalsByValue: Map<string, GrammarV2LiteralId>;
  readonly constructorIdsByShape: Map<string, GrammarV2ConstructorId>;
  readonly fieldIdsByName: Map<string, GrammarV2FieldId>;
  readonly expressionIds: { next: number };
  readonly currentRuleId: { value: GrammarV2RuleId | undefined };
}

/** Lowers grammar v2 syntax into deterministic target-neutral analysis data. */
export function analyzeGrammarV2(
  grammar: GrammarV2Document,
  options: AnalyzeGrammarV2Options = {},
): AnalyzedGrammarV2 {
  ...
  const extensions: AnalyzedGrammarV2Extension[] = [];
  for (const declaration of grammar.declarations) {
    if (declaration.kind === "extend") {
      ...
      if (!context.rulesByName.has(declaration.target)) {
        diagnostic(
          context,
          "GRAMMAR_V2_UNKNOWN_EXTENSION_TARGET",
  ...
  const reachableTokens = new Set<GrammarV2TokenId>();
  const reachableLiterals = new Set<GrammarV2LiteralId>();
...
function grammarName(grammar: GrammarV2Document): string {
...
function collectTopLevelName(
  ...
  declaration: GrammarV2Declaration,
): void {
  if (declaration.kind === "rule") {
...
function collectMode(
  context: AnalyzerContext,
  declaration: GrammarV2ModeDeclaration,
): void {
...
function collectToken(
  context: AnalyzerContext,
  declaration: GrammarV2TokenDeclaration,
  fallbackModeId: GrammarV2ModeId,
): void {
...
function collectRule(
  ...
  let sync: AnalyzedGrammarV2Expression | undefined;
...
function isSyncOnlyRule(rule: GrammarV2Rule): boolean {
...
function analyzeExpression(
  ...
  const islandId: GrammarV2ExpressionIslandId =
    context.expressionIslands.length;
  let ruleId: GrammarV2RuleId = -1;
...
interface TokenPatternAnalysis {
  readonly token: AnalyzedGrammarV2Token;
  readonly dfa: Dfa;
}

function analyzeTokenPatterns(
  context: AnalyzerContext,
  reachableTokens: ReadonlySet<GrammarV2TokenId>,
): void {
  ...
  for (const token of context.tokens) {
    ...
    try {
      ...
      diagnostic(
        context,
        "GRAMMAR_V2_INVALID_TOKEN_PATTERN",
  ...
  for (let leftIndex = 0; leftIndex < patterns.length; leftIndex++) {
    ...
    for (
      ...
      try {
        ...
        diagnostic(
          context,
          "GRAMMAR_V2_TOKEN_OVERLAP_LIMIT",
  ...
  for (let tokenIndex = 0; tokenIndex < patterns.length; tokenIndex++) {
    ...
    const previousDfas = patterns.slice(0, tokenIndex).map((pattern) =>
      pattern.dfa
    );
...
function tokenLabel(token: AnalyzedGrammarV2Token): string {
...
function validateSyncExpression(
  ...
  if (expression.kind === "ref") {
    ...
    if (expression.reference.kind === "rule") {
      diagnostic(
        context,
        "GRAMMAR_V2_INVALID_SYNC_REFERENCE",
  ...
  diagnostic(
    context,
    "GRAMMAR_V2_INVALID_SYNC_DECLARATION",
...
function validateConstructorFields(
  ...
  for (const field of fields) {
    ...
    if (existing !== undefined) {
      diagnostic(
        context,
        "GRAMMAR_V2_DUPLICATE_CONSTRUCTOR_FIELD",
    ...
    diagnostic(
      context,
      "GRAMMAR_V2_INVALID_CONSTRUCTOR_FIELD",
...
function validateExpressionOperators(
  context: AnalyzerContext,
  operators: readonly GrammarV2ExpressionOperator[],
): void {
  const operatorsByShape = new Map<string, SourceSpan>();
  const precedenceByToken = new Map<string, GrammarV2ExpressionOperator>();
  for (const operator of operators) {
    ...
    if (existing !== undefined) {
      diagnostic(
        context,
        "GRAMMAR_V2_DUPLICATE_PRECEDENCE",
    ...
    if (conflicting.precedence !== operator.precedence) {
      diagnostic(
        context,
        "GRAMMAR_V2_PRECEDENCE_CONFLICT",
...
function terminalPatternKey(pattern: GrammarV2TerminalPattern): string {
...
function analyzeRuleFacts(context: AnalyzerContext): void {
  const nullable = new Map<GrammarV2RuleId, boolean>();
  const productive = new Map<GrammarV2RuleId, boolean>();
  for (const rule of context.rules) {
...
function detectPrattLeftRecursion(context: AnalyzerContext): void {
  const nullableRules = new Map<GrammarV2RuleId, boolean>();
  for (const rule of context.rules) {
    nullableRules.set(rule.id, rule.nullable);
  }
  const firstRefsByRule = new Map<
    GrammarV2RuleId,
    readonly GrammarV2RuleId[]
  >();
  for (const rule of context.rules) {
    firstRefsByRule.set(
      rule.id,
...
function firstRuleRefs(
  expression: AnalyzedGrammarV2Expression,
  nullableRules: ReadonlyMap<GrammarV2RuleId, boolean>,
): readonly GrammarV2RuleId[] {
  const refs = new Set<GrammarV2RuleId>();
...
function findRuleCycle(
  ruleId: GrammarV2RuleId,
  targetRuleId: GrammarV2RuleId,
  firstRefsByRule: ReadonlyMap<GrammarV2RuleId, readonly GrammarV2RuleId[]>,
  visited: Set<GrammarV2RuleId>,
): readonly GrammarV2RuleId[] | undefined {
...
function expressionProductive(
  expression: AnalyzedGrammarV2Expression,
  productiveRules: ReadonlyMap<GrammarV2RuleId, boolean>,
): boolean {
  if (expression.kind === "literal") {
...
function collectReachableRules(
  rules: readonly AnalyzedGrammarV2Rule[],
  rootRule: GrammarV2RuleId | undefined,
): ReadonlySet<GrammarV2RuleId> {
  const reachable = new Set<GrammarV2RuleId>();
...
function selectRootRule(
...
): GrammarV2RuleId | undefined {
  if (requested !== undefined) {
    const root = context.rulesByName.get(requested);
    if (root === undefined) {
      diagnostic(
        context,
        "GRAMMAR_V2_UNKNOWN_ROOT",
  ...
  if (context.rules.length === 0) {
    diagnostic(
      context,
      "GRAMMAR_V2_MISSING_ROOT",
      "Grammar v2 analysis requires at least one rule.",
...
function visitExpression(
  expression: AnalyzedGrammarV2Expression,
  visitor: (expression: AnalyzedGrammarV2Expression) => void,
): void {
  visitor(expression);
  if (expression.kind === "field" || expression.kind === "constructor") {
...
function nextExpressionId(context: AnalyzerContext): GrammarV2ExpressionId {
