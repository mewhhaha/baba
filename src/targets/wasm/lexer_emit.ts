...
function emitWasmLexerSource(
  ...
  const runtimeProgram = createLexerRuntimeProgram({
    transitions: contextual?.transitions ?? [],
    asciiTransitions: contextual?.asciiTransitions ?? null,
    accepts: contextual?.acceptCandidates.map((candidates) =>
      candidates[0] ?? -1
    ) ?? [],
    acceptCandidates: contextual?.acceptCandidates ?? [],
    specs,
...
import {
  ...
  getWasmParserInstanceLimits,
  lexAll,
  WasmResourceLimitError,
  writeSource,
  type ParseTraceInput,
} from "./wasm.ts";

const DEFAULT_PRESERVE_TRIVIA = ${JSON.stringify(preserveTrivia)};
const HAS_CONTEXTUAL_LEXER = ${JSON.stringify(contextual !== null)};
...
function lexInternal(
  ...
  if (includeParseStream && HAS_CONTEXTUAL_LEXER) {
  ...
  for (let index = 0; index < records.length; index += 3) {
    ...
    tokens[tokenCount] = token;
    tokenCount++;
    appendLexDiagnostic(diagnostics, lexUnexpectedCharacterDiagnostic(token));
  }
...
function appendLexDiagnostic(
  diagnostics: LexDiagnostic[],
  diagnostic: LexDiagnostic,
): void {
  const limit = getWasmParserInstanceLimits().maxDiagnostics;
  if (limit !== undefined && diagnostics.length + 1 > limit) {
