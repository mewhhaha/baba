...
export function emitParser(
  ...
  const runtimeProgram = tableRuntimeProgram
    ...
    : mergeRuntimePrograms(productionRuntimeProgram, expectedRuntimeProgram);
...
function commonConstants(values: {
  ...
  const productionReducers = values.productions.map((production) =>
    (production as { reducer: unknown }).reducer
  );
  return `const EOF_TERMINAL = ${values.bnf.eofTerminal};
const PRODUCTION_REDUCERS: readonly ReducerSpec[] = ${
    JSON.stringify(productionReducers)
  };
...
function applyAction(
  ...
  const lhs = parserProductionLhs(action.production);
  const rhsLength = parserProductionRhsLength(action.production);
  const reducer = PRODUCTION_REDUCERS[action.production];
  if (lhs === NO_PRODUCTION || rhsLength === NO_PRODUCTION || !reducer) {
    return {
      kind: "failure",
      failure: {
        ...
        diagnostic: {
          code: "PARSER_INTERNAL_ERROR",
          message: "Parser table referenced an unknown production.",
          span: currentSpan(token),
        },
      },
    };
  ...
  const rhsValues = rhsLength === 0
    ...
    : branch.values.splice(
      branch.values.length - rhsLength,
      rhsLength,
    );
  branch.states.splice(
    branch.states.length - rhsLength,
    rhsLength,
  );
  let reduced: unknown;
  try {
