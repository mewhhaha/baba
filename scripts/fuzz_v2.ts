...
function fuzzGrammarParser(random: () => number, index: number): void {
  ...
  withContext("grammar", index, grammar, () => {
    const parsed = parseGrammarV2(grammar);
...
function fuzzLexerParserRecovery(random: () => number, index: number): void {
  ...
  withContext("lexer-parser-recovery", index, source, () => {
    ...
    validateGrammarV2Parse(parser, source);
    recoverGrammarV2Parse(parser, source, 8);
  });
}

function fuzzPortablePlan(random: () => number, index: number): void {
  const parsed = parseGrammarV2(randomGrammar(random, `Portable${index}`));
  if (!parsed.grammar) {
    throw new Error("parser returned no grammar.");
  }
  const analyzed = analyzeGrammarV2(parsed.grammar);
  ...
  try {
    ...
    throw new Error(
      [
        ...
        "Persist this case under fixtures/parser-v2/ before changing semantics.",
        message,
      ].join("\n"),
    );
  }
