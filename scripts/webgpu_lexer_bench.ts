 ...
 *
 * Machine-readable output is opt-in via `--json PATH`. It is deliberately not
 * written by default and never to a fixed path: the three `results*.json` files
 * under `experiments/webgpu-lexer/` are the evidence `docs/adr/0001` cites and
 * must not be regenerated by a routine bench run.
 *
