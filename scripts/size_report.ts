...
interface SizeBudgets {
  ...
  readonly generatedExampleSnapshotBytes?: number;
  readonly largestGeneratedExampleFileBytes?: number;
}
...
function budgetChecks(
  ...
  addBudgetCheck(
    checks,
    "generatedExampleSnapshotBytes",
    report.examples.generatedBytes,
    budgets.generatedExampleSnapshotBytes,
  );
  addBudgetCheck(
    checks,
    "largestGeneratedExampleFileBytes",
    report.examples.largestGeneratedFiles[0]?.size ?? 0,
    budgets.largestGeneratedExampleFileBytes,
  );
  return checks;
}
...
function renderHumanReport(report: SizeReport): string {
  const lines = [
    ...
    `Generated example snapshot bytes: ${
